...
To demonstrate potential usage of the programs [gmtspatial], [grdvolume] and
[gmtselect] we extract a subset of the Sandwell & Smith altimetric gravity field 1 for
...
and then pass these locations to [gmtselect], which retains only the points within
200 km of Pratt. We then mask out all the data outside this radius and use [grdvolume]
to determine the combined area and (gravimetric) volumes of the chosen seamounts. Our illustration
is presented in Figure 18 which also shows the automatic labeling offered by [subplot].
