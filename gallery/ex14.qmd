...
We arrange for four panels using the [subplot] command. First we plot the distribution
and values of our raw data set (same as in Example [(12) Optimal triangulation of data]).
We choose an equidistant grid and run [blockmean] which preprocesses the data to avoid
aliasing. The dashed lines indicate the logical blocks used by [blockmean]; all points
inside a given bin will be averaged. The logical blocks are drawn from a temporary file we make on
the fly within the shell script. The processed data is then gridded with the [surface]
program and contoured every 25 units. A most important point here is that [blockmean],
[blockmedian], or [blockmode] should always be run prior to running surface,
and both of these steps must use the same grid interval. We use [grdtrend] to fit a bicubic
...
(dashed line) along the transect using [plot]
