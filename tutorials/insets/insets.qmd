...
The {{< gmtref inset >}} function can be used as an option to the `plot`, `basemap` and `grdimage`
...
But if instead of a rectangle we want to plot a symbol, let’s say, a red circle, we can call the {{< gmtref plot >}} function
