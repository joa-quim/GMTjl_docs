...
- **arg1** : In alternative to `fname` pass in a Mx3 matrix or a {{< gmtref GMTdataset type=gmttype >}} with at least 3 columns.
    `arg1` can also be a {{< gmtref GMTfv type=gmttype >}} type like those produced by the _solids_ functions (sphere, etc)
...
- **C** or **density** : -- *density=??*\
    Sets body density in _SI_. Append either a constant, the name of a grid file or a {{< gmtref GMTgrid type=gmttype >}}
...
- **F** or **track** : -- *track=xy_loc*\
    Provide *xy_loc* (file name or {{< gmtref GMTdataset type=gmttype >}}) locations where the anomaly will be computed. Note,
...
- **Tv** or **index** : -- *index=vert_file* **|** *index=D*\
    ...
    of the {{< gmtref triangulate >}} module. The form *raw_triang=D* means that we can also
    pass the data as a {{< gmtref GMTdataset type=gmttype >}}
...
{{< gmtref grdgravmag3d >}}, {{< gmtref gravprisms >}}, talwani2d, talwani3d, {{< gmtref triangulate >}}
