...
- **G** or **grid** : -- *grid={{< gmtref GMTgrid type=gmttype >}}* **|** *grid=gridfile*\
    {{< gmtref GMTgrid type=gmttype >}} (a grid type) or *gridfile* is a 2-D binary grid file with the function f(x,y).
...
- **E** or **profile** : -- *profile=*line*\ [,\ *line*,...][**+a**\ *az*][**+c**][**+d**][**+g**][**+i**\ *inc*][**+l**\ *length*][**+n**\ *np*][**+o**\ *az*][**+r**\ *radius*]*\
    ...
    (*x*/*y* for Cartesian data) or a 2-character XY key that uses the {{< gmtref text >}}-style
