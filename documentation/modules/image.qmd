...
- **D** or **pos** or **position** : -- *pos=(map=true, inside=true, outside=true, norm=true, paper=true, anchor=XX, dpi=XX, width=XX, justify=code, repeat=n, offset=XX)*\
    ...
    (see {{< gmtref text >}}). **Note**: If `inside` is used (the default) then *justify* defaults
    ...
    neither size nor *dpi* is set then we revert to the default dpi [[GMT_GRAPHICS_DPU](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-GMT_GRAPHICS_DPU)]. Optionally,
...
{{< gmtref legend >}}, {{< gmtref colorbar >}}, {{< gmtref plot >}}, [psconvert]
