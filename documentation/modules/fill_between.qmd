...
The curves are defined by the points (x, y1, y2) in matrix or {{< gmtref GMTdataset type=gmttype >}} `D1`. This creates one or
...
also be a file name of a file that can be read with {{< gmtref gmtread >}} and return a GMTdataset.html)
...
`fill_between(..., pen=...)`: Sets pen specifications for the two curves. Easiest is to use the shortcuts
   `lt`, `lc` and `ls` for the line thickness, color and style like it is used in the {{< gmtref plot >}} module.
...
is the same as used in {{< gmtref text >}}, *e.g.* `TL` means *TopLeft* and `box` controls the legend box details.
...
#| output: asis
using GMT
GMT.doc_source_links("fill_between")
```
