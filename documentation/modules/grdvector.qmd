...
- **G** or **fill** : -- *fill=fill*\
    Sets color or shade for vector interiors (see [Fill color/pattern]) [Default is no fill].
...
- **S** or **vscale** or **vec_scale** : -- *vscale=(inverse=len, length=len, scale=xx, scale_at_lat=??, refsize=size)*\
    ...
    or point); if no unit is given we use the default value that is controlled by `[PROJ_LENGTH_UNI](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNI)`. Vector
...
#| output: asis
using GMT
GMT.doc_source_links("grdvector")
```
