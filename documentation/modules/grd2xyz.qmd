...
[FORMAT_FLOAT_OUT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_FLOAT_OUT) parameter in your `gmt.conf` file or use
**par=([FORMAT_FLOAT_OUT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_FLOAT_OUT)=format,)** or choose binary
...
- **T** or **stl** or **STL** : -- *STL=true* **|** *STL=[:b][base]*\
    ...
    adjusted so that *xmin = ymin = zmin = 0*. For other adjustments, see {{< gmtref grdedit >}} and {{< gmtref grdproject >}}
...
- **W** or **weight** : -- *weight=true* **|** *weight=:a[+uunit]* **|** *weight=weight*\
    ...
    *x,y,z* only]. Choose **weight=:a** to compute weights equal to the area each node represents.
    ...
    (see [Units]). For such grids, the area varies with latitude and also sees special cases
...
lattice via {{< gmtref greenspline >}}, {{< gmtref nearneighbor >}} or {{< gmtref surface >}}, for instance.
...
[TIME_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_UNIT) and [TIME_EPOCH](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_EPOCH) in the `gmt.conf` file or on the command line. The default output
...
To edit individual values in the 2' by 2' remote AFR.nc file, dump the .nc to {{< gmtref GMTdataset type=gmttype >}}.
...
{{< gmtref grdedit >}}, {{< gmtref grdconvert >}}, {{< gmtref xyz2grd >}}
