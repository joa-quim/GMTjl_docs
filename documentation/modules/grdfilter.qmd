...
### Input Grid
- **First positional argument** or **`ingrid`**: Input grid as a file name (String) or a {{< gmtref GMTgrid type=gmttype >}} object
...
Output grid file name or let the function return a {{< gmtref GMTgrid type=gmttype >}} object.
...
1. **Input formats**: GMT.jl accepts:
   - {{< gmtref GMTgrid type=gmttype >}} objects
   - File names as strings
   - Remote GMT datasets (e.g., `"@earth_relief_05m"`)

2. **Output options**:
   - Omit `G` to return {{< gmtref GMTgrid type=gmttype >}} object
...
5. **Piping workflows**: {{< gmtref GMTgrid type=gmttype >}} objects can be piped through functions:
   ```julia
...
When the output grid type is netCDF, coordinates are labeled "longitude", "latitude", or "time" based on input attributes or `f`/`R` options. Time coordinates are stored as relative time since epoch (specified by [TIME_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_UNIT) and [TIME_EPOCH](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-TIME_EPOCH)).

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("grdfilter")
```

## See Also

{{< gmtref grdfft >}}, {{< scatter grdmath >}}, {{< scatter grdproject >}}, {{< scatter grdsample >}}, {{< scatter grdgradient >}}
