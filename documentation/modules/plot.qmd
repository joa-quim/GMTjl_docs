...
- **legend** and **label** : -- *legend="thelabel"* **|** *label=thelabel* **|** *legend=(label="thelabel", pos=position, box=??, fontsise=?, font=?)*\
   ...
   `pos` option in {{< gmtref legend >}} that allows also to plot the legend outside of the figure space. The `box` option
   may take two forms (refer to {{< gmtref legend >}} for more details): (1) use `box=:none` to not plot the legend box or,
