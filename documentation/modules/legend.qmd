...
annotation font and size in effect (i.e., [FONT_ANNOT_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_PRIMARY))

![](/assets/seislegend.png){width="500" .center}
...
can be changed by appending `justify` followed by a 2-char justification code *justify* (see {{< gmtref text >}}).
...
- **F** or **box** : -- *box=(clearance=val, fill=color, inner=true, pen=pen, rounded=true, shaded=XX)*\
   Without further options, draws a rectangular border around the scale using [MAP_FRAME_PEN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_FRAME_PEN); specify a different pen with `pen=pen` (see [Pen attributes]).
...
- **B** or **colorbar** : *colorbar=(name=`cptname`, offset=`val`, height=`val` [, extra="..."])*\
   This record will plot a horizontal color bar, {{< gmtref colorbar >}} -style in the middle, starting at `offset` from the
   left edge, and of the given `height`. You may add any additional {{< gmtref colorbar >}} options as well as
   ...
   **monochrome**, **dpi**, **nolines** **zfile** and **view** may be appended as *optional arguments* at the end of
   the record. See {{< gmtref colorbar >}} for details on all modifiers and options.
...
- **D** or **hline** : *hline=(pen=`pen`, offset=`val`)*\
   ...
   line and the left and right frame sides [0]. If no pen is given we use [MAP_GRID_PEN_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_GRID_PEN_PRIMARY), and if `pen` is set
...
- **H** or **header** : *header=(text=`txt`, [font=`font`])*\
   ...
   default to size and fonttype of [FONT_TITLE](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_TITLE).
...
- **L** or **label** : *label=(label=`text`, justify=`code` [,font=`font`])*\
   ...
   `code` is one of :L, :C or :R. If `font` is not used we default to size and fonttype of [FONT_TITLE](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_TITLE)
...
- **P** or **paragraph** : *paragraph=true* **|** *paragraph=`options`*\
   Start a new text paragraph by specifying all the parameters needed (see {{< gmtref text >}} **paragraph** record description).
...
- **S** or **symbol** : *symbol=(marker=`name`, [dx_left=`val`,] size=`val` [fill=`fill`, pen=`pen`] [, dx_right, label=`text`])*\
   Plots the selected symbol with specified diameter, fill, and outline (see {{< gmtref plot >}}). The symbol is centered
   at `dx_left` from the left margin of the column, with the optional explanatory *label* starting `dx_right` from
   the margin, printed with [FONT_ANNOT_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_PRIMARY). If `dx_left` is not given then it is automatically computed from
   ...
   **symbol** then we simply skip to the next column. Three {{< gmtref plot >}} symbols may take special modifiers:
   *front* (**f**), *quoted line* (**q**) and *vector* (**v**). You can append modifiers to the symbol and affect how
   the fronts, quoted lines
   and vectors are presented (see {{< gmtref plot >}} man page for modifiers). The module will determine default settings
   ...
   that as the symbol size (again, see {{< gmtref plot >}} for details on the arguments needed).

- **T** or **text[xx]** : *text[xx]=`text`*\
   One or more of these **text** records with *paragraph-text* printed with [FONT_ANNOT_PRIMARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_PRIMARY). To specify special
