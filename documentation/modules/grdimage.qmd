...
created from the grid using {{< gmtref grdgradient >}} and, optionally, modified by {{< gmtref grdhisteq >}}. A third
...
Higher resolution images can be obtained by using the **dpi** option. To obtain the resampled value (and hence
...
is controlled with the **interp** option.
...
- **coast** : -- *coast=true* **|** *coast=(...)*\
   Call the {{< gmtref coast >}} module to overlay coastlines and/or countries. The short form *coast=true* just
...
- **colorbar** : -- *colorbar=true* **|** *colorbar=(...)*\
   Call the {{< gmtref colorbar >}} module to add a colorbar. The short form *colorbar=true* automatically adds a
