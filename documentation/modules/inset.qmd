...
- **D** or **pos**  or **position**or **inset_box** or **insetbox** : -- *position=(map=true, inside=true, outside=true, norm=true, paper=true, anchor=XX, size=XX, width=XX, justify=code, offset=XX)*\
    ...
    by using **justify=??** where *??* stands for a 2-char justification code *justify* (see {{< gmtref text >}}).
...
- **C** or **clearance** : -- *clearance=val* **|** *clearance=(left=val, right=val, bott=val, bottom=val, top=val))*\
    ...
    *left* and *right* or **tb=val** for both *top* and *bott*. No *side* means all sides. Alternatively, if all sides
    ...
    Append units as desired  (**c**m, **i**nch, or **p**oint) [Default is set by [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT)].

- **F** or **box** : -- *box=(clearance=val, fill=color, inner=true, pen=pen, rounded=true, shaded=XX)*\
    Without further options, draws a rectangular border around the map inset using [MAP_FRAME_PEN](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_FRAME_PEN). The following
...
quite nice features that allow creating elaborated figures with very short commands. The [Figure insets]
...
- **inset** -- *inset=(data, zoom=(...), coast=(...), position=(...), box=(...), clearance=(...))*\
    ...
    - **data** An image, a grid a table ({{< gmtref GMTdataset type=gmttype >}}) or a file name that can be automatically read by
    {{< gmtref gmtread >}}. Depending on the data type an _x,y_ plot or an image is displayed inside the inset window. 
    ...
    - **coast** With this argument we can call the {{< gmtref coast >}} command with all of its normal arguments. And
    furthermore, we can even call the {{< gmtref plot >}} (or any of its avatars) to add line/symbol plots over the
