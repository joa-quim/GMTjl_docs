...
- **C** or **dist2pt** : -- *dist2pt=(pts=pointfile, dist=dist)* **|** *dist2pt=(pts=(lon,lat), dist=dist)*\
    ...
    **dist2pt=(pts=(lon,lat), dist=dist)** instead of *pointfile*. Distances are Cartesian and in user units;
    ...
    specified is 0. (see [Units]). Alternatively, if **region** and **proj** are used then
    geographic coordinates are projected to map coordinates (in cm, inch, or points, as determined by
    [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT)) before Cartesian distances are compared to *dist*.
...
- **F** or **polygon** : -- *polygon=polygonfile* **|** *polygon=matrix*\
    ...
    file *polygonfile* (or {{< gmtref GMTdataset type=gmttype >}} with *in-memory* data). For spherical polygons (lon, lat),
...
- **L** or **dist2line** : -- *dist2line=(line="linefile", dist=dist [,ortho=true])* **|** *dist2line=(line=[pt1; pt2], dist=dist [,ortho=true])*\
    ...
    multiple-segment file *linefile* or *in-memory* matrix or {{< gmtref GMTdataset type=gmttype >}}. If *dist* is zero
    ...
    specify **colinfo=:f** to indicate spherical distances append a distance unit (see [Units]).
    Alternatively, if **region** and **proj** are used then geographic coordinates are projected to map
    coordinates (in cm, inch, m, or points, as determined by [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT)) before Cartesian
...
That means that options like **colinfo=:oT** and settings like [FORMAT_FLOAT_OUT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_FLOAT_OUT) and [FORMAT_GEO_OUT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_GEO_OUT)
...
units (cm, inch, or points, as determined by [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT)).
...
the new boundary points at the segment ends you must use {{< gmtref gmtspatial >}} instead.
...
{{< include ../common_opts/explain_inside.qmd >}}
...
#| output: asis
using GMT
GMT.doc_source_links("gmtselect")
```
