...
- **C** or **canvas** : -- *canvas=...*\
    ...
    **Note**: Your [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT) setting determines if **movie** sets you up to work with the SI or US canvas
...
- **G** or **fill** : -- *fill=color* | *fill=(fill=color, pen=pen)*\
    ...
    outline with *pen* [no outline]. See [Setting color] and [Pen attributes] for extend color and pen selections.
...
- **L** or **label** : -- *label=str* (not yet translated to keywords)\
    ...
    *col* from the trailing text in *timefile* (first word is 0). **Note**: If you use **-Lc**
    ...
    [FORMAT_DATE_MAP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_DATE_MAP) and [FORMAT_CLOCK_MAP](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_CLOCK_MAP). By default,
    ...
    Append **+f** to use a specific *font* [[FONT_TAG](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_TAG)].
...
- **P** or **progress** : -- *progress=(indicator=char, annot="...", font=font, justify=tag, offset=(dx,dy), width=..., fill=color, Fill=color, pen=pen, Pen=pen)*\
    ...
    [[FONT_ANNOT_SECONDARY](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FONT_ANNOT_SECONDARY) scaled as needed]. Append **offset=dx** or **offset=(dx,dy)** to offset indicator in direction
...
{{< include ../common_opts/opt__x.qmd >}}
...
require full paths unless their directories were already included in the [DIR_DATA](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-DIR_DATA) setting.
...
24 x 13.5 cm (16:9) or 24 x 18 cm (4:3). If your [PROJ_LENGTH_UNIT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PROJ_LENGTH_UNIT) setting is inch then the custom canvas sizes
...
Note the canvas selection via [PS_MEDIA](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-PS_MEDIA), the matching region and projection, and
the zero location of the origin.

Basemap Frames
--------------

Some map projections will by default draw a *fancy* map frame; this feature is under the control of [MAP_FRAME_TYPE](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-MAP_FRAME_TYPE).
...
![](/assets/GMT_movie_progress.png){width="500" .center}
