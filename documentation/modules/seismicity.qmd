...
This module is a mix of {{< gmtref plot >}}, {{< gmtref coast >}} and {{< gmtref legend >}} functionalities. So not all (fine)
...
- `data`: The default is to make a seismicity map but if the `data` option is used (containing whatever)
    we return the data in a {{< gmtref GMTdataset type=gmttype >}}
...
#| output: asis
using GMT
GMT.doc_source_links("seismicity")
```
...
{{< gmtref plot >}}, {{< gmtref coast >}}, {{< gmtref legend >}}