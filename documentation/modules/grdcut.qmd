...
(but see **extend**). If in doubt, run {{< gmtref grdinfo >}} to check range. Alternatively, define the subregion
...
{{< gmtref grdpaste >}}, which will join together two grid files (not images) along a common edge.
...
- **E** or **rowlice** or **colslice** : -- *rowlice=coord* **|** *colslice=coord*\
    ...
    given directive. Note:
        1- Input file must be a 3-D netCDF cube, and this option resturns a {{< gmtref GMTgrid type=gmttype >}}.
...
Suppose you have used {{< gmtref surface >}} to grid ship gravity in the region
