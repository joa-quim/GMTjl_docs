...
data with {{< gmtref blockmean >}}, {{< gmtref blockmedian >}}, or {{< gmtref blockmode >}} to avoid
...
- **D** or **breakline** : -- *breakline="bfile"* **|** *breakline="bfile+z[level]"*\
    Use *x, y, z* data in the *bfile* file (or a {{< gmtref GMTdataset type=gmttype >}}) as a `soft breakline`. A `soft breakline`
...
{{< include ../common_opts/opt_-Grid.qmd >}}
...
- **M** or **mask** : -- *mask=max_radius*\
    ...
    a data constraint are set to NaN. Append a distance unit (see [Units]) if needed.
...
- **Q** or **suggest** : -- *suggest=true* **|** *suggest=:r*\
    ...
    {{< gmtref grdsample >}} or {{< gmtref grdcut >}} on the output of **surface**. Alternatively,
...
data sets; see {{< gmtref greenspline >}} (for modest data sets) or {{< gmtref sphinterpolate >}}, or
...
you run {{< gmtref blockmean >}}, {{< gmtref blockmedian >}}, or {{< gmtref blockmode >}} first. If you
...
places by editing the parameter [FORMAT_FLOAT_OUT](https://docs.generic-mapping-tools.org/latest/gmt.conf.html#term-FORMAT_FLOAT_OUT) in your `gmt.conf` file prior to running the
