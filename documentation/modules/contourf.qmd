...
This module is a wrapper to {{< gmtref grdview >}}, *contour* and {{< gmtref grdcontour >}} and as such it works with two
different kinds of input data. If input is a grid (either a grid file name or a {{< gmtref GMTgrid type=gmttype >}} object) it will make a filled
contour with *grdview* and optionally  overlay contours by calling {{< gmtref grdcontour >}}. If, on the other hand,
the input data is table data file or a Mx3 array (or {{< gmtref GMTdataset type=gmttype >}}), it first compute a Delaunay triangulation and makes
...
The input data set to be filled/contoured. It can be 2-D gridded data {{< gmtref GMTgrid type=gmttype >}}) or a Mx3 table (or GNTdataset). In these cases
...
Not exactly required but convinient, the second input argument can be a {{< gmtref GMTcpt type=gmttype >}} object. The advantage of this is that it
...
- **A** or **annot** : -- *annot=annot\_int* **|** *annot=(int=annot\_int, disable=true, single=true, labels=labelinfo)*\
   *annot\_int* is annotation interval in data units; it is ignored if contour levels are given in a file.
   ...
   Alternatively do *annot=(single=true, int=val)* to plot *val* as a single contour. The optional *labelinfo* controls the specifics of the label formatting and consists of a named tuple with the following control arguments [Label formatting]
...
- **C** or **cont** or **contour** or **contours** or **levels** : -- *cont=cont\_int*\
   ...
   1. If ``cont_int`` is a *{{< gmtref GMTcpt type=gmttype >}}* object (or a string and has the suffix ".cpt" and can be opened as a file).
   ...
   2. If ``cont_int`` is a constant or an array it means plot those contour intervals. This works also to draw
	...
	If no **contour** option and no *{{< gmtref GMTcpt type=gmttype >}}* are passed then for grid a default color map is computed and all of
	those automatically contours are drwan. Also, no *{{< gmtref GMTcpt type=gmttype >}}* and **contour=[array]** computes a cmap with only the
	contour values specified in *array*. When passing a *Mx3* array or a *{{< gmtref GMTdataset type=gmttype >}}* the default behavior is
	basically the same.
...
- **G** or **labels** : -- *labels=()*\
   ...
   controlling algorithms as explained in [Label formatting]
