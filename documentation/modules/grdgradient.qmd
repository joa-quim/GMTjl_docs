...
If you don't know what **norm** options to use to make an intensity file for {{< gmtref grdimage >}} and {{< gmtref grdview >}}, a good first try is **norm=e0.6**.
...
If you want to make several illuminated maps of subregions of a large data set, and you need the illumination effects to be consistent across all the maps, use the **norm** option and supply the same value of *sigma* and *offset* to {{< gmtref grdgradient >}} for each map. A good guess is *offset* = 0 and *sigma* found by `grdinfo` **-L2** or **-L1** applied to an unnormalized gradient grd.
...
#| output: asis
using GMT
GMT.doc_source_links("grdgradient")
```
