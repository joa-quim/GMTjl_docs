...
multiple segment file that can be send to {{< gmtref plot >}} to draw the triangulation network.
...
For geographic data with global or very large extent you should consider {{< gmtref sphtriangulate >}}
...
- **C** or **slope_grid** : -- *slope_grid=slpfile*\
    Read a slope grid (in degrees), either a file or a {{< gmtref GMTgrid type=gmttype >}}, and compute the
...
- **G** or **save** or **outgrid** or **outfile** : -- *outgrid=[=ID][+ddivisor][+ninvalid][+ooffset|a][+sscale|a][:driver[dataType][+coptions]]*\
    ...
    **inc** options this one is automatically selected. Now, if you want to save the result in a grid
    ...
    of all gridding routines) or instead use {{< gmtref sphtriangulate >}}. For natural nearest neighbor
    gridding you must add **voronoi=:pol**. See other modules that also write files, such as
    {{< gmtref surface >}}, to see further options to this.
...
- **L** or **index** : -- *index=indexfile[*+b*]*\
    Give name of file or a {{< gmtref GMTdataset type=gmttype >}} with previously computed Delaunay information.
...
{{< include ../common_opts/opt__r.qmd >}}  (Only valid with **outgrid**).
...
{{< include ../common_opts/explain_float.qmd >}}
...
#| output: asis
using GMT
GMT.doc_source_links("triangulate")
```
...
{{< gmtref contour >}},
{{< gmtref sphdistance >}},
{{< gmtref sphinterpolate >}},
{{< gmtref sphtriangulate >}},
{{< gmtref surface >}}
