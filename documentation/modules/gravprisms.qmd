...
![](/assets/GMT_seamount_prisms.png){width="500" .center}
...
- **fname** or **arg1** : -- *fname=file_name* **|** *arg1=matrix|D*\
    The file name or a matrix/[GMTdatset](GMTdatset.html) describing the prisms with record format *x y z_lo z_hi* [ *dx dy* ] [ *rho* ],
...
**H** or **radial_rho** : -- *radial_rho=(height=?, lowhigh=(rho_l, rho_h), boost=?, pressure_rho=?, power=?)*\
    ...
    See {{< gmtref grdseamount >}} for more details.

**L** or **base** : -- *base=fname|grid*\
    Give a file name or a {{< gmtref GMTgrid type=gmttype >}} of the base surface grid for a layer we wish to approximate 
...
**N** or **track** : -- *track=trackfile* **|** *track=matrix|GMTdatset*\
    ...
    {{< gmtref GMTdataset type=gmttype >}}. If **level** is not set then *track* must have 3 columns and we take the *z*
...
#| output: asis
using GMT
GMT.doc_source_links("gravprisms")
```
...
[gravmag3d], {{< gmtref grdgravmag3d >}}, grdseamount, talwani2d, talwani3d
