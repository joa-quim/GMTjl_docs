...
are better done using {{< gmtref mapproject >}} as **project** is strictly spherical.
...
(Note that `Dgc` could now be used as input for {{< gmtref grdtrack >}}, etc. ).
...
about the pole of the projection. You now can use {{< gmtref grdcontour >}} on `Gflow` to draw lines of
