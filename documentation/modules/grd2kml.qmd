...
selected (i.e., no contours specified) then the PNG tiles are written directly from {{< gmtref grdimage >}}.
Otherwise, we must first make a PostScript plot that is then converted to raster image via [psconvert].
...
- **N** or **prefix** : -- *prefix=name*\
    ...
    directory where all referenced KML files and raster images will be written [GMT_Quadtree](GMT_Quadtree.html).
...
{{< include ../common_opts/use_cpt_grd.qmd >}}
...
- **I** or **shade** or **shading** or **intensity** : -- *shade=grid* **|** *shade=azim* **|** *shade=(azimuth=az, norm=params, auto=true)*\
   ...
   call to {{< gmtref grdgradient >}}; use `shade=az` or ``shade=(azimuth=az, norm=params)`` to specify azimuth
...
- **W** or **contours** : -- *contours="cfile|pen[+sscale/limit]"*\
    ...
    are overlain via calls to {{< gmtref grdcontour >}}. **Note**: The contour pen width(s)
...
{{< include ../common_opts/opt_n.qmd >}}

Quadtree building
-----------------

We extend the input grid vi {{< gmtref grdcut >}} to obtain a square dimension that can be repeatedly divided
...
The intensity grid can be created from the data grid using {{< gmtref grdgradient >}} and, optionally,
modified by {{< gmtref grdmath >}} or {{< gmtref grdhisteq >}}. Custom intensity grids built with several
different illumination angles can be combined with {{< gmtref grdmath >}}. For a single illumination
