...
- `varname`: When netCDF files have more than one 2D (or higher) variables use `varname` to pick
  ...
  Remember that the ``layout`` member of the {{< gmtref GMTgrid type=gmttype >}} type informs about memory layout.
...
{{< gmtref gmtwrite >}}, {{< gmtref gdalread >}}
```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("gmtread")
```
