...
resort to the {{< gmtref coast >}} module and its ability to clip the dry and wet parts of the Earth. In the first
...
- `f1`: The name of the module that will be used to plot the data. Normally, this should be either {{< gmtref grdimage >}}
  or {{< gmtref grdview >}}, but {{< gmtref plot >}} can in principle be used as well.
- `arg1`: The input data to be used by the `f1` module. Normally, a {{< gmtref GMTgrid type=gmttype >}} a {{< gmtref GMTimage type=gmttype >}}, a
  [GMTdatset](GMTdatset.html) or a string with the file name of the data to use.
...
Other than the above options, we can pass in `kw` options to be consumed in the {{< gmtref coast >}} module and that will
...
{{< gmtref coast >}}, {{< gmtref grdimage >}}, {{< gmtref grdview >}}, {{< gmtref plot >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("sealand")
```
