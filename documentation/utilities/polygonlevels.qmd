...
- `ids`: is a string Vector or Matrix with the ids (attribute names) of the {{< gmtref GMTdataset type=gmttype >}} D.
...
- `idvals`: is a {{< gmtref GMTdataset type=gmttype >}} with the `text` field containing the ids to match against the `ids` strings.
            The first column of `id_vals` must contain the values to be used in `vals`. This is a comodity
            function when both the `ids` and `vals` are store in a {{< gmtref GMTdataset type=gmttype >}}.
...
{{< gmtref inwhichpolygon >}}, {{< gmtref getbyattrib >}}, {{< gmtref coast >}}, [plot3]

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("polygonlevels")
```
