...
return a grid {{< gmtref GMTgrid type=gmttype >}} type. Alternatively to `hdr`, provide a pair of vectors, `x` & `y`, with the
...
Create Float {{< gmtref GMTgrid type=gmttype >}} with size, coordinates and increment determined by the contents of the HDR var. This
...
Where F is a function and X,Y the vectors coordinates defining it's domain. Creates a Float32 {{< gmtref GMTgrid type=gmttype >}} with
...
creates a Float32 {{< gmtref GMTgrid type=gmttype >}}. Example: ``G = mat2grid("sombrero")``


See Also
--------

{{< gmtref mat2img >}}, {{< gmtref mat2ds >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("f")
```
