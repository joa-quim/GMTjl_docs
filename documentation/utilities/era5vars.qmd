...
This function returns a JSON formatted string that can be used as an input to the {{< gmtref ecmwf >}} function `params` option.
See the {{< gmtref listecmwfvars >}} function for a list of available variables.
...
{{< gmtref ecmwf >}}, {{< gmtref era5time >}}, {{< gmtref listecmwfvars >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("era5vars")
```
