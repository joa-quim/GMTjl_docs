...
This method takes a {{< gmtref GMTimage type=gmttype >}} cube, normally satellite data of ``UInt16`` type created with the ``RemoteS`` package,
and returns a {{< gmtref GMTimage type=gmttype >}} cube of ``UInt8`` of the principal components in decreasing order of explained variance.
...
This method takes a {{< gmtref GMTgrid type=gmttype >}} cube and returns another grid, of type `DT` (``Float32`` by default), with principal
