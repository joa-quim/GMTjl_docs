...
```julia
# 3 methods for generic function "autocov" from GMT:
 [1] autocov(x::AbstractVecOrMat{<:Real}; demean)
 [2] autocov(x::AbstractMatrix{<:Real}, lags::AbstractVector{<:Integer}; demean)
 [3] autocov(x::AbstractVector{<:Real}, lags::AbstractVector{<:Integer}; demean)
```
