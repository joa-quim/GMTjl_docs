...
of the {{< gmtref GMTdataset type=gmttype >}} `shapes`. The `GI` array is modified in place.

- `GI`: A grid ({{< gmtref GMTgrid type=gmttype >}}) or image ({{< gmtref GMTimage type=gmttype >}}) type that will be modified by
  applying `fun` to the elements that fall inside the polygons of `shapes`.

- `shapes`: A vector of {{< gmtref GMTdataset type=gmttype >}} containing the polygons inside which the elements if `GI`
...
#| output: asis
using GMT
GMT.doc_source_links("rasterzones!")
```
