...
Take a 2D `mat` array and convert it into a {{< gmtref GMTdataset type=gmttype >}}. `x` is an optional coordinates vector (must have the
...
- `ref:` Pass in a reference {{< gmtref GMTdataset type=gmttype >}} from which we'll take the georeference info as well as `attrib` and `colnames`
...
Take a 2D `mat` array and convert it into a {{< gmtref GMTdataset type=gmttype >}}. Pass in a reference {{< gmtref GMTdataset type=gmttype >}} from which we'll take
...
Create a multi-segment {{< gmtref GMTdataset type=gmttype >}} (a vector of {{< gmtref GMTdataset type=gmttype >}}) from matrices passed in a vector-of-matrices `mat`.
...
Cut a {{< gmtref GMTdataset type=gmttype >}} D with the indices in INDS but updating the colnames and the Timecol info.
...
{{< gmtref mat2grid >}}, {{< gmtref mat2img >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("mat2ds")
```
