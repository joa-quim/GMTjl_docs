...
- `replicate`: A NamedTuple with one or more of the following fields: `centers`, a Mx3 Matrix with the centers of the copies;
  ...
  option to assign a color of each copy (default is the ``1:size(centers, 1)``); `cmap`, a {{< gmtref GMTcpt type=gmttype >}} object; `scales`,
...
{{< gmtref extrude >}}, {{< gmtref flatfv >}}, {{< gmtref fv2fv >}}, {{< gmtref grid2tri >}}, {{< gmtref revolve >}}, {{< gmtref surf2fv >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("replicant")
```
