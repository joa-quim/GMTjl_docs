...
Compute 2-D streamlines as a 2-D matrix (in fact, a {{< gmtref GMTdataset type=gmttype >}}) of vector fields.
The inputs `U` and `V` are {{< gmtref GMTgrid type=gmttype >}}s with the `x` and `y` velocity components, and `startX` and
...
#| output: asis
using GMT
GMT.doc_source_links("streamlines")
```
