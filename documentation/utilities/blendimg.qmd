...
Blend the RGB `color` {{< gmtref GMTimage type=gmttype >}} with the `shade` intensity image (normally obtained with
[gdaldem]). The `new` argument determines if we return a new RGB image or update the `color` argument.
...
- `img1`, `img2`: A RGB ({{< gmtref GMTimage type=gmttype >}}{UInt8, 3}) or an indexed images ({{< gmtref GMTimage type=gmttype >}}{UInt8, 2}).
...
- `new` If true returns a new {{< gmtref GMTimage type=gmttype >}} object, otherwise it changes the `img1` content.

Returns ``nothing`` if `new` option has not been used or a {{< gmtref GMTimage type=gmttype >}}, either RGB or indexed
