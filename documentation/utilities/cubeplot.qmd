...
Plot images on the sides of a cube. Those images can be provided as file names, or {{< gmtref GMTimage type=gmttype >}} objects.
...
- `region`: The limits extents that will be used to annotate the *x,y,z* axes. It uses the same syntax as all
  other modules that accept this option (*e.g.* {{< gmtref coast >}}). It defaults to "0/9/0/9/0/9"
...
Make a 3D plot of a 3D {{< gmtref GMTgrid type=gmttype >}} (a cube) with a top view perspective from the 4rth quadrant (only one implemented).
...
- `cmap, colormap, cpt, colorscale`: Pass in a {{< gmtref GMTcpt type=gmttype >}} colormap to be used to paint the vertical walls and
...
- `top`: An optional {{< gmtref GMTgrid type=gmttype >}} or the file name of a {{< gmtref GMTgrid type=gmttype >}} to be used to create the top wall of the cube. If, instead,
  a {{< gmtref GMTimage type=gmttype >}} is passed we plot it directly (grids are converted to images using default colormaps or one passed via `topcmap`).
  The default is to use the cube's first slice as the top wall.

- `topshade`: Only used when the `top` option was used to pass a {{< gmtref GMTgrid type=gmttype >}} (or the name of one). When `true`, the top wall
...
The {{< gmtref cubeplot >}} function let us easily plot images on the sides of a cube. That function can also be used
...
![](/assets/Vp_mosaic.png){width="280" .center}
...
![](/assets/Vp_Vs_cubes.png){width="1000" .center}
...
![](/assets/Vp_inset.png){width="800" .center}

See Also
--------

{{< gmtref slicecube >}}, {{< gmtref xyzw2cube >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("cubeplot")
```
