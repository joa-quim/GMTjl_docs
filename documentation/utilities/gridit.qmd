...
- `indata`: Alternative source dataset in the form of a {{< gmtref GMTdataset type=gmttype >}}, a Mx3 matrix or a GDAL dataset.
...
- `method`: The interpolation method name. One of (GMT): "surface" or "minimum curvature", "triangulate",
  ...
  "lowest" will compute those amounts inside each *rectangular* cell.
    ...
    - Note that there is some overlap between the diverse methods. For example, the GMT's {{< gmtref nearneighbor >}}
...
- `preproc`: This option only applies to the `method=:surface` and means that the data is previously passed
  through one of ``block*`` modules to decimate the data in each cell as strongly advised by the {{< gmtref surface >}}
  program. `preproc=true` will use {{< gmtref blockmean >}}. To use any of the other two, pass its name as value.
  *e.g.* `preproc="blockmedian"`.

Returns
-------

A {{< gmtref GMTgrid type=gmttype >}} or nothing if file was writen on disk.
...
{{< gmtref blockmean >}}, {{< gmtref blockmedian >}}, {{< gmtref surface >}}, {{< gmtref triangulate >}}, {{< gmtref nearneighbor >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("gridit")
```
