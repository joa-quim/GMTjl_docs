...
Crop a subregion of a grid ({{< gmtref GMTgrid type=gmttype >}}) or a image ({{< gmtref GMTimage type=gmttype >}}). The subregion is
specified with the ``limits`` or ``region`` keyword; the specified range must not exceed the range of the input.
This function differs from {{< gmtref grdcut >}} in the sense that it doesn't call the GMT lib and works only on
