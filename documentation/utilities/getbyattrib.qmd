...
Take a {{< gmtref GMTdataset type=gmttype >}} vector and return only its elements that match the condition(s) set by the `kw` keywords.
...
Either a vector of {{< gmtref GMTdataset type=gmttype >}}, or a vector of Int with the indices of the segments that match th
query condition. Or `nothing` if the query results in an empty {{< gmtref GMTdataset type=gmttype >}} 
...
{{< gmtref inwhichpolygon >}}, {{< gmtref polygonlevels >}}
```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("getbyattrib")
```
