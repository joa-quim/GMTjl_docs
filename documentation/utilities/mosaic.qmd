...
- `lon` & `lat`:
  ...
  - Instead of two arguments, pass just one containing a {{< gmtref GMTdataset type=gmttype >}} obtained with the ``geocoder`` function.
  ...
  - Yet another alternative is to pass either a {{< gmtref GMTgrid type=gmttype >}} or a {{< gmtref GMTimage type=gmttype >}} with a valid
  ...
  - Finaly, all of the above options can be skipped if the keyword `region` is used. Note that this option is
    the same as in, for example, the {{< gmtref coast >}} module. And that means we can use it with
    {{< gmtref earthregions >}} arguments. _e.g._  ``region="IT"`` is a valid option and will get the tiles
...
- `tilesmesh` or `meshtiles` or `mesh`: Return a {{< gmtref GMTdataset type=gmttype >}} with the mesh of tiles.

Returns
-------
- `I`: A {{< gmtref GMTimage type=gmttype >}} element or the output of the `quadonly` or `mesh` options explained above.
