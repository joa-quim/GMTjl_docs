...
Generate an anaglyph image from the input {{< gmtref GMTgrid type=gmttype >}} `G` or grid's file name `fname`.
...
feeling of depth. The second method, that generates 2 3D views using the {{< gmtref grdview >}} program
...
### Args
- `G`: The input {{< gmtref GMTgrid type=gmttype >}} or filename of data to be processed.
...
- `R`: Region of interest when reading a grid from disk (default: entire grid).
   Ignored when `G` is a {{< gmtref GMTgrid type=gmttype >}}.

- `view3d`: If true, selects an alternative and slower method that generates 2 3D views using
   the {{< gmtref grdview >}} program and construct the anaglyph from those two images (default: false).

- `zsize`: z-axis size of the 3D view. Same as in {{< gmtref grdview >}} (default: 4 cm).
...
### Returns
An anaglyph {{< gmtref GMTimage type=gmttype >}} suitable for viewing with red-cyan glasses.

### Credits
The method that uses the grid's gradient is based on an ancient program called ManipRaster by Tierrt Souriot.
The second method, the one that uses the {{< gmtref grdview >}} program, was proposed by Tim Hume in the GMT forum.
