...
Write XYZ data to a LIDAR laz (laszip compressed) or las format file. The second form is used to write a {{< gmtref GMTgrid type=gmttype >}}
to a laz file. This is a rather more compressed format that waht is achieved with netCDF, but slightly lossy.
Use {{< gmtref lazread >}} or {{< gmtref gmtread >}} to read the laz file back into a GMTgrid.
...
- `G`: A {{< gmtref GMTgrid type=gmttype >}} object.
...
{{< gmtref lazread >}}, {{< gmtref gmtread >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("lazwrite")
```
