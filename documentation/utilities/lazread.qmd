...
- `do_grid`: If true, the output is a {{< gmtref GMTgrid type=gmttype >}}. This implicitly sets `out="xyz"` and `type=Float32`.
...
- `do_image`: If true, the output is a {{< gmtref GMTimage type=gmttype >}}. This implicitly sets `out="xyRGB"` and of course it will
...
indicating what data is stored in output. `grd` is a {{< gmtref GMTgrid type=gmttype >}} if `grid` is true or we read a _grided_ laz file.
`img` is a {{< gmtref GMTimage type=gmttype >}} if `image` was set to true and the laz file contains RGB data.
`ds` is a {{< gmtref GMTdataset type=gmttype >}} with the xyz data, and `dsv`is a vector of {{< gmtref GMTdataset type=gmttype >}} if input opion `out` was set
...
{{< gmtref lazwrite >}}, {{< gmtref gmtread >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("lazread")
```
