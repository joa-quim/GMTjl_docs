...
polygons of the {{< gmtref GMTdataset type=gmttype >}} `shapes`. 

### Arguments

* `GI`: A grid ({{< gmtref GMTgrid type=gmttype >}}) or image ({{< gmtref GMTimage type=gmttype >}}) type uppon which the statistics will be computed by applying the 
  `fun` function to the elements that fall inside the polygons of `shapes`.

* `shapes`: A vector of {{< gmtref GMTdataset type=gmttype >}} containing the polygons inside which the elements if `GI` will be assigned
...
#| output: asis
using GMT
GMT.doc_source_links("zonal_statistics")
```
