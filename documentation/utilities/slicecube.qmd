...
Take a slice of a multi-layer {{< gmtref GMTimage type=gmttype >}}. Return the result still as a {{< gmtref GMTimage type=gmttype >}}. `layer` is the z slice number.

```julia
slicecube(G::GMTgrid, slice::Union{Int, AbstractVector{<:Int}}; axis="z")
```

Extract a slice from a {{< gmtref GMTgrid type=gmttype >}} cube.

- `slice`: If it is an Int it will return a {{< gmtref GMTgrid type=gmttype >}} corresponding to that layer.
...
Take a slice of a {{< gmtref GMTgrid type=gmttype >}} or {{< gmtref GMTimage type=gmttype >}} in an oblique direction. Take the cube's layer `slice` and rotate it
by `angle` degrees about the `axis`. This one can only be `axis=:x` or `axis=:y`. Depending on the data
type of input a different output is produces. If `GI` is a {{< gmtref GMTgrid type=gmttype >}}, the output is 2 GMTgrids: one with `z`
levels and the other with cube's z levels along that plane. On the other hand, if GI isa {{< gmtref GMTimage type=gmttype >}} the
first output is similar to previus case but the second will be a {{< gmtref GMTimage type=gmttype >}}. In this case the `cmap` option
...
Get the fourth layer of the multi-layered `I` {{< gmtref GMTimage type=gmttype >}} object 
...
{{< gmtref cubeplot >}}, {{< gmtref xyzw2cube >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("slicecube")
```
