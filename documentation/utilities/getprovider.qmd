...
### Arguments
  ...
  * `name`: Name of the tile provider. Currently available are "Bing" (the default), "Google", "OSM", "Esri", "Nimbo".
    Optionally, the `name` can be a tuple of two strings, where the first string is the provider name and the second 
    string is the variant name (see the `variant` bellow).
  * The `name` argument can also be a `Provider` type from the TileProviders.jl package. For example, after
    importing TileProviders.jl, `provider = NASAGIBSTimeseries()` and next pass it to `getprovider`.
...
#| output: asis
using GMT
GMT.doc_source_links("getprovider")
```
