...
### Args
- `mag`: {{< gmtref GMTgrid type=gmttype >}} or filename of the magnetic field (nT) for the direct problem or magnetization (A/m) for the
   inverse problem.
...
### Returns
A {{< gmtref GMTgrid type=gmttype >}} with the computed magnetic fiels or magnetization.
...
{{< gmtref parkergrav >}}, {{< gmtref gravfft >}}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("parkermag")
```
