...
options. But contrary to the [Monolithic](monolitic.html) usage, the one letter *GMT* option syntax may be
...
*{{< gmtref GMTgrid type=gmttype >}}* type. The ``plot()`` function allows a no limits setting, in which case it will default to the
data's bounding box.
...
return it to Julia as a {{< gmtref GMTimage type=gmttype >}} type.
...
As referred in the [Monolithic](monolitic.html) section, we have two programs to do read and writing. Their
module names are *gmtread* and *gmtwrite*. These modules allow to import and export any of the GMT
data types to and from external files. For instance, to save the grid *G* stored into a {{< gmtref GMTgrid type=gmttype >}} type
