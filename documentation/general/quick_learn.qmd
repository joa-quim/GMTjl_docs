...
we created a {{< gmtref GMTcpt type=gmttype >}} object that resides only in Julia memory space and passed it directly via the **C** option.
...
how to manipulate import/create data stored in {{< gmtref GMTgrid type=gmttype >}}, {{< gmtref GMTimage type=gmttype >}},
{{< gmtref GMTdataset type=gmttype >}}, {{< gmtref GMTcpt type=gmttype >}} and {{< gmtref GMTps type=gmttype >}} objects.
...
Here we introduced also the use of a module that does not exist in GMT, {{< gmtref imshow >}}, but one that
is in fact a mockup made with {{< gmtref grdimage >}}, {{< gmtref grdview >}}, {{< gmtref plot >}} and {{< gmtref plot3d >}}
...
GMT syntax. Let's, see the example of the {{< gmtref plot >}} (`psxy`) module
...
a {{< gmtref GMTcpt type=gmttype >}} argument (the example above), or a Tuple with the three (R,G,B) color elements with values between 0 and 255,
...
The {{< gmtref plot >}} command is hugely vast, so a series of *avatars* have been derived from it. Namely, `lines`,
