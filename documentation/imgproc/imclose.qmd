...
with the structuring element {{< gmtref strel >}}. Later case is faster but it is only available for binary images, where by
...
### Returns
A new {{< gmtref GMTimage type=gmttype >}} of the same type as `I` with the closing applied.
...
![](/assets/imclose_nocrash.png){width="600" .center}

```{julia}
#| echo: false
#| output: asis
using GMT
GMT.doc_source_links("imclose")
```

### See Also

{{< gmtref imdilate >}}, {{< gmtref imerode >}}, {{< gmtref imopen >}}, {{< gmtref strel >}}
