...
Converts a connected components structure `CC` returned by {{< gmtref bwconncomp >}} into a binary image `BW`.
...
| `bbox` | The bounding boxes as a vector of {{< gmtref GMTdataset type=gmttype >}} |
...
A new {{< gmtref GMTimage type=gmttype >}}, returned as a logical array of the same size as `CC.image_size`.
...
This example also shows a problem with the {{< gmtref bwconncomp >}} function when the BoundingBoxes
...
#| output: asis
using GMT
GMT.doc_source_links("cc2bw")
```

## See Also

{{< gmtref bwconncomp >}}, {{< gmtref binarize >}}