...
- `marker`: The image to be reconstructed (will hold the reconstructed image). This can be a matrix
  or a {{< gmtref GMTimage type=gmttype >}} with Boolean or UInt8 types.

- `mask`: The mask image. Types ({{< gmtref GMTimage type=gmttype >}} or matrix) are Boolean or UInt8.
...
### Returns
- A new {{< gmtref GMTimage type=gmttype >}} (or Matrix)
...
#| output: asis
using GMT
GMT.doc_source_links("imreconstruct")
```

\1

{{< gmtref fillsinks >}}, {{< gmtref imfill >}}
